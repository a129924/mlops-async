...
Use this template for `api-client-porting-planner` output. Keep unknown values explicit and do not infer missing source behavior.
...
Use these rows to update a migration map or equivalent tracker when one is in scope. If no tracker file is in scope, keep these rows in the planner handoff artifact.
