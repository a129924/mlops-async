# request-gate-casmanagement-get-table technical specification
...
- 實作邊界固定在
  `tests/unit/request_contract/casmanagement_table_get_request_gate/**`，不得修改
  `src/**`。
...
- harness 必須保留 topic fixture root fallback，不可跳出
  `tests/unit/request_contract/casmanagement_table_get_request_gate/fixtures`
...
- reserved characters in `tableName` 必須 percent-encoding
...
- `docs/request-shape-priority-workflow/**` 僅在 final `release` 補 current truth，
  但不得把 queue 外 surface 誤改成 queue 內完成。
- 本 topic creator / publish-in-progress implementation 不得落地 stable-library changes。

## Validation Commands

```bash
uv run pytest tests/unit/request_contract/casmanagement_table_get_request_gate -v
uv run ruff check tests/unit/request_contract/casmanagement_table_get_request_gate
uv run pyright tests/unit/request_contract/casmanagement_table_get_request_gate/conftest.py tests/unit/request_contract/casmanagement_table_get_request_gate/test_get_table_request_contract.py
```
