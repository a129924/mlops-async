...
`core/headers.py` now serves as the shared request-header policy surface for JSON-domain requests and token-endpoint requests, while `Requester` still owns auth merge and Authorization-conflict behavior.
