...
def _normalize_job_id(job_id: object) -> str:
    if not isinstance(job_id, str) or not job_id.strip():
        raise BlockedTopicScopeError("Only a non-empty jobId string is allowed in this topic.")
    return job_id
...
class _FakeHttpClient:
    ...
    async def request_json(self, *args: object, **kwargs: object) -> object:
        raise AssertionError(
            "JobExecution state request-contract tests should call request(), not request_json()"
        )
...
class JobExecutionStateContractHarness:
    def __init__(self) -> None:
        self._transport = _FakeHttpClient()
        self.client = JobExecutionJobsStateClient(
...
@pytest.fixture
def job_execution_state_contract() -> JobExecutionStateContractHarness:
    return JobExecutionStateContractHarness()
