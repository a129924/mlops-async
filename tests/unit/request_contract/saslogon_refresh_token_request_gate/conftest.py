"""Request-shape helpers for SASLogon refresh token request gate."""
...
def _reject_client_credentials_variants(
    client_id: object | None,
    client_secret: object | None,
) -> None:
    if client_id is not None:
        raise BlockedTopicScopeError("client_id is blocked in this topic.")
    if client_secret is not None:
        raise BlockedTopicScopeError("client_secret is blocked in this topic.")
...
class SasLogonRefreshTokenClient:
    """Minimal refresh-token client used only for this isolated request gate."""
    ...
    def refresh_access_token(
        ...
        *,
        ...
        grant_type: object = "refresh_token",
        ...
        if not isinstance(payload, dict):
            raise TypeError("Refresh token request-gate response must decode to a JSON object.")
        return SimpleNamespace(**payload)


class SasLogonRefreshTokenContractHarness:
    def __init__(self) -> None:
        self.client = SasLogonRefreshTokenClient()
...
@pytest.fixture
def saslogon_refresh_token_contract() -> SasLogonRefreshTokenContractHarness:
    return SasLogonRefreshTokenContractHarness()
