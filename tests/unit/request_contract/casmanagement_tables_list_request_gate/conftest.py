"""Request-shape helpers for CASManagement list tables request gate."""
...
LIST_TABLES_PATH_PREFIX = "/casManagement/dataSources/cas~fs~cas-shared-default~fs~"
ALLOWED_LIMIT = 1000
ALLOWED_START = 0
...
class CASManagementTablesListClient:
    """Minimal list-tables client used only for this isolated request gate."""
    ...
    def list_tables(
        ...
        *,
        caslib: object,
        limit: object = ALLOWED_LIMIT,
        start: object = ALLOWED_START,
        include_default_query: object = True,
        extra_query: object | None = None,
        body: object | None = None,
        endpoint_variant: object = "list_tables",
    ) -> object:
        normalized_caslib = _normalize_non_empty_string(caslib, "caslib")
        normalized_limit = _normalize_limit(limit)
        normalized_start = _normalize_start(start)
        _reject_missing_canonical_query(include_default_query)
        ...
        response = self._session.request(
            "GET",
            f"{BASE_URL}{LIST_TABLES_PATH_PREFIX}{encoded_caslib}/tables",
            headers={
        ...
        if not isinstance(payload, dict):
            raise TypeError("CAS list_tables request-gate response must decode to a JSON object.")
        return SimpleNamespace(**payload)


class CASManagementTablesListContractHarness:
    def __init__(self) -> None:
        self.client = CASManagementTablesListClient()
...
@pytest.fixture
def casmanagement_tables_list_contract() -> CASManagementTablesListContractHarness:
    return CASManagementTablesListContractHarness()
