...
| REQ-006 | TC-BC-001 | Changed test-path evidence stays under `tests/**`, while no `tests/**` changes also pass | red |
...
| TC-BC-001 | state/side effects | state/side effects | 驗證 `tests` scope guard 採寬鬆版：若存在 `tests/**` 變更，其 evidence path 皆位於 `tests/**`；若無 `tests/**` 變更亦可通過。 |
...
| TC-BC-001 | `tests/unit/test_importlib_policy_guard.py` | `test_tc_bc_001_req_006_tests_scope_evidence_guard_is_still_red` |
...
### Scenario 5: Loose tests-scope evidence guard
- **Given**: topic 可能有或沒有 `tests/**` 變更
- **When**: 套用 `REQ-006` guard
- **Then**: 只有在觀察到 `tests/**` 變更時才檢查其 evidence path 邊界；沒有 `tests/**` 變更時不因 guard 本身失敗
